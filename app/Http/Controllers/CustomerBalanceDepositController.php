...
class CustomerBalanceDepositController extends Controller
{
    ...
    {
        $customerBalanceDeposits = CustomerBalanceDeposit::with('balance.customer')->get();
        Log::info('Customer Balance Deposits:', $customerBalanceDeposits->toArray());
        // Mengubah data menjadi format yang lebih sesuai untuk API
        $response = $customerBalanceDeposits->map(function($customerBalanceDeposit) {
            return [
                'id' => $customerBalanceDeposit->id,
                'customer_id' => $customerBalanceDeposit->balance->customer->id ?? null, 
                'customer_name' => $customerBalanceDeposit->balance->customer->customer_name ?? '-', 
                'deposit_amount' => $customerBalanceDeposit->deposit_amount, 
                'deposit_date' => $customerBalanceDeposit->deposit_date,
            ];
        });
        return response()->json([
            'deposits' => $response,
            'debug' => $customerBalanceDeposits->toArray(),
        ]);
     ...
     */
    public function show(CustomerBalanceDeposit $customerBalanceDeposit)
    {
        return $customerBalanceDeposit->load('balance');
    }
